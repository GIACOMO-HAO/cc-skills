...
| 脚本 | 作用 |
|------|------|
| `${CLAUDE_SKILL_DIR}/scripts/api-dispatch.sh` | API 直调（DeepSeek / Qwen / GLM / 豆包 / MiniMax）|
| `${CLAUDE_SKILL_DIR}/scripts/codex-appserver.mjs` | Codex App Server 协议直连（只读 / 写模式）|
| `${CLAUDE_SKILL_DIR}/scripts/gemini-supervisor.sh` | Gemini CLI 调用 + retry / fallback / circuit breaker |
| `${CLAUDE_SKILL_DIR}/scripts/kimi-dispatch.sh` | Kimi 调用 + endpoint 路由 + API fallback |
